...
define("RACE_GOBLIN", 256);
define("RACE_HIGHELF", 512);
...
function races($race)
{
    ...
    if ($race == (RACE_HUMAN | RACE_ORC | RACE_DWARF | RACE_NIGHTELF | RACE_UNDEAD | RACE_TAUREN | RACE_GNOME | RACE_TROLL | RACE_GOBLIN | RACE_HIGHELF) || $race == 0) {
        return array('side' => 3, 'name' => LOCALE_BOTH);
    } elseif ($race == (RACE_ORC | RACE_UNDEAD | RACE_TAUREN | RACE_TROLL | RACE_GOBLIN)) {
        return array('side' => 2, 'name' => LOCALE_HORDE);
    } elseif ($race == (RACE_HUMAN | RACE_DWARF | RACE_NIGHTELF | RACE_GNOME | RACE_HIGHELF)) {
        ...
        if ($race & RACE_HIGHELF) {
            ...
            if ($races['name'])
                $races['name'] .= ', ';
            $races['name'] .= LOCALE_HIGHELF;
        }
        if ($race & RACE_GOBLIN) {
            (($races['side'] == 1) or ($races['side'] == 3)) ? $races['side'] = 3 : $races['side'] = 2;
            if ($races['name'])
                $races['name'] .= ', ';
            $races['name'] .= LOCALE_GOBLIN;
        }
